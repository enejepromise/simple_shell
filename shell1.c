...
void print_prompt(void)
{
	if (isatty(STDIN_FILENO))
		write(STDOUT_FILENO, "PromiseShell$ ", 9);
}
...
char *read_input(void)
{
	...
	lineptr = NULL;
	lineptr_size = 0;
	nread = getline(&lineptr, &lineptr_size, stdin);
...
/**
  * execute_command - Executes the input from the lineptr
  * @input: The argument from the lineptr
